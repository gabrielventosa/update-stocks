...
def main():
    ...
    CLEAR_COLORS = ['Nude','Fiusha','Maquillaje','Verde', 'Naranja', 'Blanco', 'Lila', 'Plata', 'Cobalto', 'Aqua', 'Azúl Rey',
     'Oro', 'Mica Camel', 'Beige', 'Mica Dorado', 'Eléctrico', 'Cielo', 'Rosa', 'Dorado', 'Camel', 'Mostaza']
    DARK_COLORS = ['Negro', 'Negro Satín']
    ...
    try:
        ...
        items = []
        orders = getPendingOrders(MAGENTO_SITE, bearer)
        for o in orders['items']:
            order_number = o['entity_id']
            total_pieces = 0
            pieces_by_size = {}
            pieces_by_color = {}
            dark_pieces = {}
            clear_pieces = {}
            items += getMagentoOrderItems(MAGENTO_SITE, bearer,str(order_number))['items']
        ...
        for i in items:
            ...
            if color in CLEAR_COLORS:
                if size not in clear_pieces:
                    clear_pieces[size] =0
                clear_pieces[size] += qty_backordered
            else:
                if size not in dark_pieces:
                    dark_pieces[size] =0
                dark_pieces[size] += qty_backordered

            pieces_by_size[size] += qty_backordered
            pieces_by_color[color] += qty_backordered
            #print(size)
            #print(f'Total pieces: {total_pieces}')
            #print(f'Clear pieces: {clear_pieces}')
            #print(f'Dark pieces: {dark_pieces}')
        print(json.dumps(pieces_by_size, indent=4, sort_keys=True))
        print(json.dumps(pieces_by_color, indent=4, sort_keys=True))

        print(f'Clear pieces: ')
        print(json.dumps(clear_pieces, indent=4, sort_keys=True))
        print(f'Dark pieces: ')
        print(json.dumps(dark_pieces, indent=4, sort_keys=True))
        ...
        print(f'Total requested pieces: {total_pieces}')
...
def getPendingOrders(url, bearer):
    """
    url = url+quote('/index.php/rest/V1/orders/' + orderId )
    """
    url = url+ '/index.php/rest/V1/orders?' + \
